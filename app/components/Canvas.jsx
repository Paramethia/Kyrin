...
const Canvas = () => {
    ...
    showCentipede = window.innerWidth >= 700;
    ...
    window.addEventListener("resize", function() {
        ...
        particles = [];
        init();
        showCentipede = window.innerHeight >= 700;
    ...
    */
    ...
    // Color change button functioning

    const colorChangeB = document.getElementById("colour-change");
    let colors = [
        ...
        "rgba(252, 107, 2, 0.7)", "rgba(236, 151, 6, 0.7)", "rgba(249, 129, 42, 0.7)", "rgba(188, 86, 2, 0.7)", // orange, honey, tangerine, ginger
        ...
        "rgba(255, 229, 180, 0.7)" // peach
    ];
    if (colorChangeB) {
    colorChangeB.onclick = () => {
        // Choose random colors
        var randomColor = Math.floor(Math.random() * colors.length);
        //if (randomColor === color) randomColor = Math.floor(Math.random() * colors.length);
        //console.log("Picked color:", colors[randomColor]);
        color = colors[randomColor];
        colors.splice(randomColor, 1);
        console.log("Colors left:", colors.length);
        // Pick colors in ascending order
        /*
        var c = 0;
        color = colors[c];
        c++;
    ...
    function drawMainCircle() {
        ...
        ctx.strokeStyle = "rgba(255, 229, 180, 0.7)";
        ctx.beginPath();
        var size = 12;
        ...
        for (var index = 0; index < particles.length; index++) {
            ...
            for (var i = index; i < particles.length; i++) {
                ...
                if (distance <= 100) {
                    ...
                    // Draw lines between every particle when they are close
                ...
                // Draw lines between particles and mouse / main particle when they are close
                
                let disx =  particle.x - Input.mouse.x;
                let disy = particle.y - Input.mouse.y;
                let dist = Math.sqrt(disx * disx + disy * disy);
                
                if (dist <= 155) {
                    ctx.beginPath();
                    ctx.strokeStyle = `rgba(48, 48, 48, ${dist / 155})`;
                    ctx.moveTo(particle.x, particle.y);
    ...
    //setupCentipede(.5,100,128);//Literal centipede
    //setupSquid(2,8);//Spidery thingy
    let size
    let legs;
    let tail;
    if (windowWidth >= 1920) {
    ...
    return (<canvas style="display: none"></canvas>)
}
